...
import { TextField, Text, Icon } from '@bosch/react-frok';
import useDebounce from '@hooks/useDebounce';
import './assignUser.style.css';
...
interface AssignUserMultiSelectProps {
  id: string;
  label: string;
  placeholder?: string;
  ...
  onRemoveUserCurrent: (value: string) => void;
  onRemoveUserAdding: (value: string) => void;
  onRestoreUserRemoved: (value: string) => void;
  ...
  getSelectedCurrentUserLabel?: (value: string) => string;
  getSelectedAddingUserLabel?: (value: string) => string;
  getSelectedRemovingUserLabel?: (value: string) => string;
...
const AssignUserMultiSelect: React.FC<AssignUserMultiSelectProps> = ({
  id,
  label,
  placeholder = 'Search...',
  selectedValues,
  ...
  getSelectedCurrentUserLabel,
  getSelectedAddingUserLabel,
  getSelectedRemovingUserLabel,
  ...
  const handleResultSelect = (item: any) => {
    ...
    if (!selectedValues.includes(value)) {
      onSelect(value, getOptionLabel(item));
    }
    setShowResults(false);
    setSearchTerm('');
  };
  ...
  return (
    <div className="space-y-2">
          ...
          {selectedValues.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {selectedValues.map((value) => (
                <div key={value} className="flex items-center rounded-md bg-bosch-blue-60 px-3 py-1.5 text-bosch-white">
                  <span className="mr-2">
                    {getSelectedCurrentUserLabel ? getSelectedCurrentUserLabel(value) : value}
                  </span>
                  <button
                    type="button"
                    className="text-bosch-white hover:text-bosch-red"
                    onClick={() => onRemoveUserCurrent(value)}
                  >
                    ✕
                  </button>
                </div>
            ...
            <Text className="italic text-bosch-gray-60">No any assignee</Text>
          )}
        </div>
      </div>
      {/* Adding Items */}
          ...
          {addedValues.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {addedValues.map((value) => (
                <div
                  key={value}
                  className="flex items-center rounded-md bg-bosch-green-60 px-3 py-1.5 text-bosch-white"
                >
                  <span className="mr-2">{getSelectedAddingUserLabel ? getSelectedAddingUserLabel(value) : value}</span>
                  <button
                    type="button"
                    className="text-bosch-white hover:text-bosch-red"
                    onClick={() => onRemoveUserAdding(value)}
                  >
                    ✕
                  </button>
                </div>
            ...
            <Text className="italic text-bosch-gray-60">No any added assignee</Text>
          )}
        </div>
      </div>
      {/* Removing Items */}
      <div className="mt-2">
        <div>
          <p>Assignee(s) will be removed from current: </p>
        </div>
        <div className="assignee-list">
          {removedValues.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {removedValues.map((value) => (
                <div key={value} className="flex items-center rounded-md bg-bosch-red-60 px-3 py-1.5 text-bosch-white">
                  <span className="mr-2">
                    {getSelectedRemovingUserLabel ? getSelectedRemovingUserLabel(value) : value}
                  </span>
                  <button
                    type="button"
                    className="text-bosch-white hover:text-bosch-red"
                    onClick={() => onRestoreUserRemoved(value)}
                  >
            ...
            <Text className="italic text-bosch-gray-60">No any removed assignee</Text>
          )}
        </div>
      </div>
    </div>
...
export default AssignUserMultiSelect;
